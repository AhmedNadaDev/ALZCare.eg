...
const PatientPage = () => {
  ...
  const [status, setStatus] = useState('System Standby'); // Face Recognition temporarily disabled
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Patient System</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className={`h-2 w-2 rounded-full ${cameraActive ? 'bg-green-500 animate-pulse' : 'bg-gray-500'}`} />
              <span className="text-sm text-gray-400">{cameraActive ? 'Live' : 'Standby'}</span>
            </div>
          </div>
        </div>
      </nav>
      ...
      <main className="pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">
            Welcome {user?.fullName || `${user?.firstName || ''} ${user?.lastName || ''}`.trim() || 'Patient'}
            {user?.patientNumber ? ` (${user.patientNumber})` : ''} to the patient assistance system
          </p>
        </div>
        ...
        {/* Face Recognition UI elements temporarily disabled
          ...
          {!cameraActive && (
            <div className="absolute inset-0 flex items-center justify-center text-gray-500">
              <div className="text-center">
                <CameraIconLg />
                <p className="mt-4 text-gray-400">Camera is currently disabled</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white/[0.03] rounded-2xl border border-white/[0.08] p-6">
          ...
          <ul className="space-y-3 text-gray-400">
            <li className="flex items-start gap-3">
              ...
              <span>This system provides assistance for patients and their families.</span>
            </li>
            <li className="flex items-start gap-3">
              <span className="text-purple-400 font-bold">2.</span>
              <span>Certain features like real-time face recognition may be temporarily disabled for maintenance.</span>
            </li>
          </ul>
        </div>
      </main>
